...
# Here, we form the 1-dimensional histogram and KDE to estimate the distributions of amygdala and acc, respectively. 
# We will decide on a suitable number of bins so we can see the shape of the distribution clearly,
# & set an appropriate kernel bandwidth h > 0.
...
# Next, we form a 2-dimensional histogram for the pairs of variables (amygdala, acc) &
# determine the suitable number of bins so the shape of the distribution is clearly visable.
...
nbin = 30        # we can change the number of bins in each dimension
fig = plt.figure()
...
# here we dropped the normalization factor in front of the kernel function,
# but we  can add it back. It is just a constant scaling.
...
# Next, we use kernel-density-estimation (KDE) to estimate the 2-dimensional density function 
#  of (amygdala, acc) The first component is the definition: Two variables are independent when 
#  the distribution of one does not depend on the the other. In practice, we can check this 
#  by using the conditional distribution. If the probabilities of one variable remains fixed, 
#  regardless of whether we condition on another variable, then the two variables are independent. 
#  Otherwise, they are not.
# The second component involves sampling: We do not often have access to the probabilities 
# that generate a variable. We have access only to data attained through sampling. This means 
#  that there is some room for error. The observed conditional frequencies do not have to be exactly 
#  equal for the data to be independent: they need only be roughly equal. We can quantify 
#  what it means to be roughly equal, but, here, we’ll use a less rigorous, graphical approach.

# Estimate mutual information for a discrete target variable.
# Mutual information (MI) [1] between two random variables is a non-negative value, which measures 
# the dependency between the variables. It is equal to zero if and only if two random variables 
...
# distances as described in [2] and [3]. Both methods are based on the idea originally proposed 
# in [4].  MI can be used for univariate features selection, read more in the User Guide."
# https://scikit-learn.org/stable/modules/generated/sklearn.feature_selection.mutual_info_regression.html#sklearn.feature_selection.mutual_info_regression

# Here, we explore whether the two variables (amygdala, acc) likely to be independent or not? 
#  (NOTE:  This involves prerequisite knowledge. From traditional probability and statistics, 
#  how do you show that two random variables are independent? Once you can answer that, 
#  then you need to visually represent that rule somehow and test for independence.)  
# 
#     -Probability 
# 
...
# ###  C.i  mutual info:  Mutual information  returns zero for independent variables and higher 
#  values the more dependence there is between the variables.
# The computation of the mutual information function is based on the application of nonparametric 
#  techniques, based on entropy estimation from  k-nearest neighbors distances. This methodology 
#  is drawn from the foundational concepts initially introduced by L. F. Kozachenko, & N. N. Leonenko in 1987 [1].  
#  Mutual information (MI) serves as a mathematical measure quantifying the dependency between two variables. 
#  This measure assumes non-negative values. MI attains a value of zero if the two variables are independent, with larger MI values indicative of greater interdependency.
# 
...
# C.  ii.  Permutation:  A p-value greater than 0.05 means that deviation 
...
# C.i. Spearman test
# Interpreting Correlation Metric: If the value is near ± 1, then it said to be a perfectly correlation: as one variable increases, 
...
# The p-value for a hypothesis test whose null hypothesis is that two samples have 
# no ordinal correlation
...
# Next, we consider the variable orientation and consider conditional 
# distributions. We will plot the estimated conditional distribution of amygdala 
...
# acc:  p(acc j orientation = c), c = 2; : : : ; 5 using KDE. (Note: the conditional 
# distribution can be understood as fitting a distribution for the data with the same
# orientation. Thus we will plot 8 one-dimensional distribution functions in total.)
# Based on these results, we will infer whether or not the conditional distribution of 
# amygdala and acc, respectively, are different for c = 2; : : ; 5? This is a type of
...
# 
# We  also fill out the conditional sample mean for the two variables:
# Remark: As you can see this analysis, we can extract so much more information from
...
# Here, we will consider the variable orientation. We will estimate the
...
# 5. Next, we will use two-dimensional KDE to achieve the goal; get an appropriate kernel
# band-width h > 0. We will show the two-dimensional KDE. 
# KDE of amygdala and acc w/ Political orientation = n
...
ax2 = df.plot.scatter(x='amygdala', y='acc',c='orientation',colormap='rainbow') 
